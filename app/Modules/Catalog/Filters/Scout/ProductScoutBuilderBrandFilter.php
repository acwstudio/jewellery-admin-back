...
class ProductScoutBuilderBrandFilter implements ProductScoutBuilderFilterContract
{
    ...
    {
        $builder->filter(
            Query::term()->field('brand_id')->value($id)
        );
    }
    ...
    {
        $builder->filter(
            Query::terms()->field('brand_id')->values($ids)
        );
    }
}
