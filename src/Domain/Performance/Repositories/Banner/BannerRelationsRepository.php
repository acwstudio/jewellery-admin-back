...
final class BannerRelationsRepository extends AbstractRelationsRepository
{
    public function indexRelations(array $data): Paginator|Model
    {
        ...
        if (in_array(Banner::findOrFail($id)->{$relation}()::class, config('api-settings.to-one'))) {
            return Banner::findOrFail($id)->{$relation}()->firstOrFail();
        }
        return Banner::findOrFail($id)->{$relation}()->paginate($perPage)->appends(data_get($data, 'params'));
    }
    ...
    public function updateRelations(array $data): void
    {
         ...
         */
            ...
            data_get($data, 'model') ?? data_set($data, 'model', Banner::findOrFail(data_get($data, 'id')));
