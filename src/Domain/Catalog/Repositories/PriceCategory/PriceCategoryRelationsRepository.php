...
final class PriceCategoryRelationsRepository extends AbstractRelationsRepository
{
    ...
    public function indexRelations(array $data): Paginator|Model
    {
        ...
        if (in_array(PriceCategory::findOrFail($id)->{$relation}()::class, config('api-settings.to-one')))
        {
            return PriceCategory::findOrFail($id)->{$relation}()->firstOrFail();
        }

        return PriceCategory::findOrFail($id)->{$relation}()->paginate($perPage)->appends(data_get($data, 'params'));
    }
