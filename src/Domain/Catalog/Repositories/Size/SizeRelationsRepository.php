...
final class SizeRelationsRepository extends AbstractRelationsRepository
{
    public function indexRelations(array $data): Paginator|Model
    {
        ...
        if (in_array(Size::findOrFail($id)->{$relation}()::class, config('api-settings.to-one')))
        {
            return Size::findOrFail($id)->{$relation}()->firstOrFail();
        }

        return Size::findOrFail($id)->{$relation}()->paginate($perPage)->appends(data_get($data, 'params'));
    }
